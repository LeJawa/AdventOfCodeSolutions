...
def modified_bhk(group: list[str], source: str, target: str, distance_dict: dict[str, dict[str, int]], minimize = True) -> int :
    if len(group) == 0:
    ...
    for city in group:    
        ...
        distances.append(modified_bhk(new_group, source, city, distance_dict, minimize) + distance_dict[city][target] + distance_dict[target][city])
...
def get_happiness_dict(input: list[str]) -> dict[str, dict[str, int]]:
    ...
    for line in input:
        ...
        try:
            happiness[splits[0]][splits[-1]] = int(splits[3]) if splits[2] == 'gain' else -int(splits[3])
        except:
            happiness[splits[0]] = {splits[-1]: int(splits[3]) if splits[2] == 'gain' else -int(splits[3])}
